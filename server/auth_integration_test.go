...
// TestAuthFlow_Demo_Fresh verifies that a brand-new demo-mode server seeds the
// "demohero" HOST user and allows sign-in with the demo password ("secret").
func TestAuthFlow_Demo_Fresh(t *testing.T) {
	dir := t.TempDir()
	mode := "demo"
	prof := &profile.Profile{
		Mode:    mode,
	...
	status := ts.getStatus(t)
	require.NotNil(t, status.Host,
		"demo seed must create demohero as HOST so the Sign-In button is visible")
	require.Equal(t, "demohero", status.Host.Username)

	user := ts.signIn(t, "demohero", "secret")
	require.Equal(t, "demohero", user.Username)
}

// TestAuthFlow_Demo_StaleDB reproduces the reported bug:
//
//	"on the /auth page only the Sign-Up button is shown (no Sign-In), and
//	 signing up as 'demohero' returns 409."
//
// Root cause: when the demo SQLite DB file already existed from a previous run
// with a corrupted state (e.g. no HOST user, but "demohero" as USER), the
// server reused that stale file instead of re-seeding. The fix is to always
// delete the demo DB on startup so the seed is re-applied unconditionally.
func TestAuthFlow_Demo_StaleDB(t *testing.T) {
	dir := t.TempDir()
	mode := "demo"
	dsn := fmt.Sprintf("%s/memos_%s.db", dir, mode)

	// Arrange: build a real demo server (creates DB + seeds demohero as HOST),
	// then downgrade demohero to USER so that no HOST exists — mimicking the
	// corrupted state that triggers the bug.
	{
		prof := &profile.Profile{
			Mode:    mode,
			Port:    0,
		...
		firstServer, err := NewServer(context.Background(), prof)
		require.NoError(t, err)
		// Corrupt the state: demohero → USER (removes the only HOST).
		_, err = firstServer.db.ExecContext(context.Background(),
			`UPDATE user SET role = 'USER' WHERE username = 'demohero'`)
		require.NoError(t, err)
		firstServer.Shutdown(context.Background())
	}

	// Act: start a second demo server against the same (now stale) DB file.
	prof := &profile.Profile{
		Mode:    mode,
		Port:    0,
		Data:    dir,
	...
	// Assert: the fix must have reset the stale DB and re-seeded it.
	// "demohero" must now be HOST (Sign-In button is visible).
	status := ts.getStatus(t)
	require.NotNil(t, status.Host,
		"after demo startup the host must be set (Sign-In button must be visible)")
	require.Equal(t, "demohero", status.Host.Username)

	// Sign-in as demohero with the seed password must succeed.
	// Without the fix this was either:
	//   - returning 409 (duplicate username) on sign-up, or
	//   - failing sign-in because demohero had been downgraded to USER.
	user := ts.signIn(t, "demohero", "secret")
	require.Equal(t, "demohero", user.Username)
}
